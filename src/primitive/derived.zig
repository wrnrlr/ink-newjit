...
const Adverb2 = opmod.Adverb2;
const Adverb3 = opmod.Adverb3;
...
pub fn derived(vm: *VM, base: V, adv: Adverb, args: []const V, f: util.ApplyFn) V {
  ...
  if (args.len == 1) {
    // 1-arg adverb form: /: and \: are not valid here (they are 2-arg-only).
    const a2 = Adverb2.fromAdverb(adv) orelse return V{ .err = .rank };
    return derived2(vm, a2, base, args[0], f);
  }
  if (args.len == 2)
    return derived3(vm, Adverb3.fromAdverb(adv), base, args[0], args[1], f);
  // N>2 args for non-each adverbs: pass through to base function directly
  return f(vm, base, args);
}

fn derived2(vm: *VM, adv: Adverb2, x: V, y: V, f: util.ApplyFn) V {
  const xt = x.tag();
...
fn derived3(vm: *VM, adv: Adverb3, x: V, y: V, z: V, f: util.ApplyFn) V {
  return switch (adv) {
    .@"'" => .{ .err = .nyi },
    .@"/" => blk: {
      if (y.tag() == .i and x.arity() == 1)
        break :blk adverbs.ndo(vm, x, y.i, z, f);
    ...
    .@"\\" => blk: {
      if (y.tag() == .i and x.arity() == 1)
        break :blk adverbs.ndos(vm, x, y.i, z, f);
      if ((y.tag() == .func or y.tag() == .partial) and x.arity() >= 1)
