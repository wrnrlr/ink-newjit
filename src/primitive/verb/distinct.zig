...
fn distinctB(vm: *VM, x: V) !V {
  ...
  const res = try N(bool).init(vm.alloc, n);
  var i: usize = 0;
...
fn distinctC(vm: *VM, x: V) !V {
  ...
  const res = try N(u8).init(vm.alloc, n);
...
fn distinctI(vm: *VM, x: V) !V {
  ...
  if (data.len <= so.DEDUP_THRESHOLD) {
    ...
    const res = try N(i32).init(vm.alloc, n);
    @memcpy(res.slice(), buf[0..n]);
    return .{ .I = res };
  }
  var map = try so.buildOrderedSet(i32, vm.alloc, data);
  defer map.deinit(vm.alloc);
  const res = try N(i32).init(vm.alloc, map.count());
...
fn distinctS(vm: *VM, x: V) !V {
  ...
  if (data.len <= so.DEDUP_THRESHOLD) {
    ...
    const res = try N(u32).init(vm.alloc, n);
    @memcpy(res.slice(), buf[0..n]);
    return .{ .S = res };
  }
  var map = try so.buildOrderedSet(u32, vm.alloc, data);
  defer map.deinit(vm.alloc);
  const res = try N(u32).init(vm.alloc, map.count());
  @memcpy(res.slice(), map.keys());
  return .{ .S = res };
}
...
fn distinctF(vm: *VM, x: V) !V {
  ...
  if (data.len <= so.DEDUP_THRESHOLD) {
    ...
    const res = try N(f32).init(vm.alloc, n);
  ...
  defer map.deinit(vm.alloc);
  try map.ensureTotalCapacity(vm.alloc, data.len);
  for (data) |v| try map.put(vm.alloc, @bitCast(v), {});
  const res = try N(f32).init(vm.alloc, map.count());
...
fn distinctL(vm: *VM, x: V) !V {
  ...
  errdefer { for (res_list.items) |v| v.deinit(alloc); res_list.deinit(alloc); }
  try res_list.ensureTotalCapacity(alloc, data.len);
  for (data) |val| {
    var found = false;
