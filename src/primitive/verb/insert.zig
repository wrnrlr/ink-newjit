...
fn insert(vm: *VM, t: V, d: V) !V {
  ...
  const new_data = try N(V).init(vm.alloc, ncols);
  @memset(new_data.slice(), .blank);
  errdefer (V{ .L = new_data }).deinit(vm.alloc);
  for (0..ncols) |ci| {
    ...
    new_data.slice()[ci] = try appendToCol(vm.alloc, col, dv);
  }
  return .{ .M = try Dict.init(vm.alloc, t_cols.ref(), .{ .L = new_data }) };
}
...
fn appendToCol(alloc: Alloc, col: V, val: V) !V {
  const n = col.len();
  const res = try N(V).init(alloc, n + 1);
