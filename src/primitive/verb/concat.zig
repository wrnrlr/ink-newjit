...
pub fn apply(vm: *VM, x: V, y: V) anyerror!V {
  ...
  if (xk == .L and yk == .L) {
    ...
    const res = try N(V).init(vm.alloc, xl + yl);
  ...
  if (xk == .L) {
    ...
    const res = try N(V).init(vm.alloc, xl + ylen);
    for (x.L.slice(), res.slice()[0..xl]) |v, *r| r.* = v.ref();
    if (yk.isVec()) {
  ...
  if (yk == .L) {
    ...
    const yl = y.L.ptr.len;
    const res = try N(V).init(vm.alloc, xlen + yl);
    if (xk.isVec()) {
    ...
    switch (vk) {
      inline .I, .F, .B, .S, .C => |VK| {
        ...
        if (x_atom and y_atom) {
          const res = try N(T).init(vm.alloc, 2);
        ...
        if (x_atom) { // atom + vec
          ...
          const res = try N(T).init(vm.alloc, 1 + yv.ptr.len);
          res.slice()[0] = atomVal(VK, x);
        ...
        if (y_atom) { // vec + atom
          ...
          const res = try N(T).init(vm.alloc, xv.ptr.len + 1);
        ...
        const xv = @field(x, @tagName(VK));
        const yv = @field(y, @tagName(VK));
        const res = try N(T).init(vm.alloc, xv.ptr.len + yv.ptr.len);
        @memcpy(res.slice()[0..xv.ptr.len], xv.slice());
  ...
  const res = try N(V).init(vm.alloc, 2);
  res.slice()[0] = x.ref();
  res.slice()[1] = y.ref();
  return V{ .L = res };
}
