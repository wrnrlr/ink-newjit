...
fn unionJoin(alloc: Alloc, t1: Table, t2: Table) !V {
  ...
  const new_data = try N(V).init(alloc, ncols);
  @memset(new_data.slice(), .blank);
  errdefer (V{ .L = new_data }).deinit(alloc);
  for (0..ncols) |ci| {
...
fn catCols(alloc: Alloc, col1: V, col2: V) !V {
  const n1 = col1.len();
  const n2 = col2.len();
  const res = try N(V).init(alloc, n1 + n2);
...
fn leftJoin(alloc: Alloc, t: Table, k: UTable) !V {
  ...
  const key_col_map = try alloc.alloc(?usize, nkcols);
  ...
  const match_rows = try alloc.alloc(?usize, nrows);
  ...
  const extra_vi = try alloc.alloc(usize, nextra);
  ...
  const out_data = try N(V).init(alloc, out_ncols);
  @memset(out_data.slice(), .blank);
  errdefer (V{ .L = out_data }).deinit(alloc);
  ...
  for (0..ntcols) |ti| {
    ...
    if (vcol_idx) |vi| {
      ...
      const new_col = try N(V).init(alloc, nrows);
  ...
  const out_cols = try N(V).init(alloc, out_ncols);
