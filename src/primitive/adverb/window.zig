...
pub fn window(vm: *VM, xn: V, x: V) V {
  ...
  switch (x.tag()) {
    .C => for (0..count) |i| {
      var win = N(u8).init(vm.alloc, n) catch return V{ .err = .memory };
      @memcpy(win.slice(), x.C.slice()[i .. i + n]);
      res.slice()[i] = .{ .C = win };
    },
    .I => for (0..count) |i| {
      var win = N(i32).init(vm.alloc, n) catch return V{ .err = .memory };
      @memcpy(win.slice(), x.I.slice()[i .. i + n]);
      res.slice()[i] = .{ .I = win };
    },
    .F => for (0..count) |i| {
      var win = N(f32).init(vm.alloc, n) catch return V{ .err = .memory };
      @memcpy(win.slice(), x.F.slice()[i .. i + n]);
      res.slice()[i] = .{ .F = win };
    },
    .B => for (0..count) |i| {
      var win = N(bool).init(vm.alloc, n) catch return V{ .err = .memory };
