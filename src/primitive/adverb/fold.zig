...
fn tryGpuReduce(vm: *VM, base: V, init: ?V, x: V) !?V {
  ...
  switch (x.tag()) {
    .I => {
      ...
      const out_range = try g.allocRange(@sizeOf(i32));
      defer g.free(out_range, @sizeOf(i32));
      try g.reduceI32(r_op, out_range, x.I.gpuRange(), n);
      var buf: [4]u8 = undefined;
      try g.read(out_range, &buf);
    ...
    .F => {
      ...
      const out_range = try g.allocRange(@sizeOf(f32));
      defer g.free(out_range, @sizeOf(f32));
      try g.reduceF32(r_op, out_range, x.F.gpuRange(), n);
      var buf: [4]u8 = undefined;
      try g.read(out_range, &buf);
...
pub fn fold(vm: *VM, base: V, init: ?V, x: V, f: anytype) !V {
  if (try tryGpuReduce(vm, base, init, x)) |v| return v;
  ...
  errdefer accum.deinit(vm.alloc);
