...
// The dispatch workgroup count is derived from the shader's own OpExecutionMode
// LocalSize (see localSizeX), so any `wg` dye baked (default 64) stays in sync.
...
// Read the LocalSize x operand from the shader's OpExecutionMode (opcode 16,
// mode 17 = LocalSize), defaulting to 64 if absent. The host divides the thread
// count by this to get the dispatch workgroup count, so it always matches
// whatever workgroup size dye baked into the shader (see `wg` in lib/dye.k).
fn localSizeX(words: [*]const u32, n_words: usize) u32 {
  if (n_words < 6) return 64;
  var i: usize = 5; // skip the 5-word SPIR-V header (magic, version, gen, bound, schema)
  while (i < n_words) {
    const wc: usize = words[i] >> 16;
    const op: u16 = @truncate(words[i] & 0xFFFF);
    if (wc == 0) break; // malformed instruction — avoid an infinite loop
    if (op == 16 and i + 3 < n_words and words[i + 2] == 17) {
      const x = words[i + 3];
...
export fn gpuCompute(words_k: ?K, input_k: ?K) callconv(.c) ?K {
...

  {
    ...
    const wgx: usize = localSizeX(words, @intCast(n_words));
    const workgroups: u32 = @intCast((n + wgx - 1) / wgx);
...
var g_cwg:   std.ArrayList(u32) = .empty;                  // workgroup size (LocalSize x) per pipeline (parallel)
...
fn resetResidentCompute(alloc: std.mem.Allocator) void {
  ...
  g_cpipe.deinit(alloc); g_cbgl.deinit(alloc); g_cnb.deinit(alloc); g_cwg.deinit(alloc);
  g_sbuf = .empty;  g_ssize = .empty;
  g_cpipe = .empty; g_cbgl = .empty; g_cnb = .empty; g_cwg = .empty;
}
...
export fn gpuDispatch(pipe_k: ?K, bufs_k: ?K, nthreads_k: ?K) callconv(.c) ?K {
  ...
  defer enc.release();
  {
    ...
    pass.setBindGroup(0, bg, null);
    const wgx: usize = g_cwg.items[pidx];
    const groups: u32 = @intCast((@as(usize, @intCast(nthreads)) + wgx - 1) / wgx);
