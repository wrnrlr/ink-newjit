...
| [doc](api/doc.md) | 41 | Extracts an API reference from ink source, off the parse CST. |
| [dye](api/dye.md) | 181 | Compiles ink lambdas to SPIR-V shaders — the GPU language layer. |
...
| [fft](api/fft.md) | 11 | Fast Fourier transform — Cooley-Tukey radix-2, decimation in time. |
...
| [image](api/image.md) | 16 | Reads, writes and scales images, dispatching on the file's magic bytes. |
| [json](api/json.md) | 2 | Parses and generates JSON, columnarising object arrays into tables. |
| [kk](api/kk.md) | 26 | Compiles idiomatic whole-array k to GPU pipelines that run on resident buffers. |
...
| [llm](api/llm.md) | 20 | Chat with Anthropic and xAI models, with live token streaming and tool use. |
...
| [plot](api/plot.md) | 15 | Line charts on the canvas: two axis specs and a series in, an antialiased vector plot out. |
...
| [spirv](api/spirv.md) | 100 | The SPIR-V instruction stencils dye patches to emit shader words. |
| [stats](api/stats.md) | 7 | Descriptive statistics: moments, correlation, regression and distributions. |
| [svd](api/svd.md) | 11 | Singular value decomposition by the Jacobi-Hestenes algorithm. |
...
| [uitest](api/uitest.md) | 35 | A deterministic, replayable harness that drives a real UI app headlessly. |
