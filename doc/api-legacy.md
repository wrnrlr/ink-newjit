---
title: Libraries (legacy notes)
---

# Libraries (legacy notes)

Hand-written library notes that predate the generated reference in
[api.md](api.md).  Migrate each section into the corresponding `lib/*.k` header
or per-binding comment (see `ink tools/doc.k -check`), then delete it here.
