...
- **`$[cond; a; [stmt;stmt;…]]` is not a block:** `[…]` is DICT literal syntax, so a branch like `[3;4]` (no `key:`) is a parse error — as of 2026-07-29 it says so, with a line:col and a caret. Before that it desynced silently: an empty dict plus a stray statement, and inside `$[…]` it ate the `]` and swallowed the following statement, so trailing code vanished with exit 0. Keep `$[]` branches as single expressions; move multi-statement work into a helper function.
