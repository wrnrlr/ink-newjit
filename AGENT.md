...
- **`[…]` IS a block (progn), and `(…)` is the dict:** `[a:1;b:2;a+b]` runs every statement and yields the last one's value, so a `$[]` branch may hold multi-statement work directly — no helper function needed. Round brackets carry the literals instead: `(a:1;b:2)` dict, `([]a:1 2;b:3 4)` table, `([k:1 2]v:3 4)` keyed table, `()!()` empty dict. A `(` commits to a dict as soon as its first item reads `key:`, after which every item must be `key:value` — so `(a:1;2)` and `(1;a:2)` are both parse errors, and an assignment can no longer be a top-level item of `(…)` (put it in a progn). `[` directly after a noun is still apply/index.
- **Null is `::`, the identity verb — there is no blank type:** as in ngn/k. `@::` → `` `o ``, `` $:: `` → `"::"`, `#::` → `1`; test it with `` x~:: `` (never `` @x~` ``). It is falsy, and it is what a binding, an unset global, a side-effecting verb, `,/()` and a dict miss all evaluate to — **the REPL prints nothing for it**. An elided item *is* `::`: `(1;;2)` prints `(1;::;2)`. In brackets it is the projection gap (`f[::;2]` ≡ `f[;2]`), so null cannot be passed as a bracket argument, though juxtaposition (`@a`) applies normally. Watch the assign digram: `a::1` / `a,::x` bind, but with nothing following, `a~::` and `1,::` read `::` as the literal. This also means a mistyped name is now callable — `oops x` silently returns `x` instead of erroring.
- **`:x` in statement position is an early return:** from the enclosing lambda, in a body, a progn or a `$[]` branch — `{$[x<0;:0;x*2]}`. As a *value* (call argument, list element) `:` keeps its verb reading, so `@[v;`px;:;99.]` is unchanged. Outside a lambda `:x` is plain identity.
...
- **A GPU micro-benchmark that reports impossible speed is usually one of two scope bugs, not DCE:** (1) resources built OUTSIDE `gpu.computeRun` — there is no device yet, so `gpu.buffer` returns handle `0` and every dispatch silently no-ops; (2) a nested lambda reading an enclosing lambda's local (`go:{[] p:…; {[i] gpu.dispatch[p;…]}' !n}`) — lambdas don't capture, so `p` is blank and the FFI gets a null. **Both fail silently.** Always assert the result actually CHANGED (a chained `+1.` kernel must show n, not 0) before believing a timing.
- **`/` is a comment when spaced, an adverb when glued:** ` / ` after a noun/verb starts a comment (`x: 5  / note`), `+/x` is the fold. This one IS spacing-sensitive by design. `\` and `'` are NOT — since 2026-07-29 `sep \ str` means the same as `sep\str` and `f ' xs` the same as `f'xs` (before that, a space silently turned split into scan, which is what made `nnLoadVocab` return a 1-entry vocab and every ASR transcript come out empty).
