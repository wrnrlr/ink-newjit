...
- **Partially qualified names**: a reference is resolved against the enclosing namespaces innermost-first, peeling one segment at a time — both the `\d ns` in force and, inside a lambda, the qualified name that lambda is bound to. So in `foo.prefix:"Abc"` / `foo.format:{prefix,x}` the body's `prefix` finds `foo.prefix`, and a `::` there writes that same member. Only a member that already exists captures the name, so unrelated globals are never shadowed. Definitions still spell out the full name; only references shorten. See `doc/reference.md`, *Partially qualified names*.
